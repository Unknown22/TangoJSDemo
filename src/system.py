...
def check_system_compatibility():
    if platform.system() == "Windows" and platform.release() == '10':
        ...
        try:
            ...
            check_system_process = subprocess.Popen(['cat', '/etc/redhat-release'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = check_system_process.communicate()
            result_system = re.search(r'CentOS', out.decode())
            result_version = re.search(r'[\d]+[.]*[\d]*[.]*[\d]*[.]*', out.decode())
            if result_system is None or result_version is None:
                return False
            if result_system.group() == "CentOS" and result_version.group().split('.')[0] == '7':
                return True
